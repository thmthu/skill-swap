    ...
    async (values) => {
      try {
        ...
        refreshUserData().then(() => {
          console.log(
            "User data refreshed successfully after preference update"
          );
        });
                    ...
                    <PopoverTrigger asChild>
                      <FormControl>
                        <button
                          ...
                          className={`flex ${getInputHeight(
                            field.value
                          )} w-full items-center justify-between rounded-md border border-gray-600 bg-bg-dark px-3 py-2 text-sm text-text-dark placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50`}
                        >
